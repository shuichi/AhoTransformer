...
# お、Transformerデコーダの形状は、原論文に従っています。 
...
# セットに挑戦してみてください。
# 
# Transformerの元論文"Attention Is All You Need"では、各エンコーダ／デコーダ層は
# 「マルチヘッド・アテンション・サブレイヤ」と「位置ごとの全結合FFNサブレイヤ」
# から構成される、と定義されています。 ￼デコーダーオンリーTransformerでは、マス
# ク付き自己注意（＋残差・LayerNorm）の上に位置ごとの FFN（＋残差・LayerNorm）が
# 乗る形になります。
# 
...
# 前提と記号
# 
# トークン列は 5 個です。位置 1〜5 に並んでいるとします。
# - 位置1: 1
# - 位置2: 2
# - 位置3: 3
# - 位置4: 4
# - 位置5: 5
# 
# バッチサイズは 1 として、モデルの隠れ次元を d_model、自己注意の「キー／クエリ
# の次元」を d_k とします。行列やテンソルの形は次のような記法で書きます。
# - (B, S, d_model)
# - B: バッチサイズ（ここでは1）
# - S: シーケンス長（ここでは5）
...
# 【ステップ1：トークン ID に変換する】
#
# まず文字 a, b, c, d, e は、それぞれ語彙（ボキャブラリ）の中の整数 ID に変換さ
# れています。 
# - 1 → 2
# - 2 → 3
# - 3 → 4
...
# - 5 → 6
# 
# のように、内部的には [2, 3, 4, 5, 6] という整数列ができます。これがシーケンス
# 長 5 の「離散トークン列」です。


# 【ステップ2：トークン埋め込みベクトルと位置埋め込みを足す】
# 
...
# 足します。位置1〜5に対応するベクトルを p_1, p_2, p_3, p_4, p_5 とすると、 
# - 位置1: x_1 = e_1 + p_1
# - 位置2: x_2 = e_2 + p_2
# - 位置3: x_3 = e_3 + p_3
# - 位置4: x_4 = e_4 + p_4
...
# 
# 自己注意では、同じ入力 X から 3 種類のベクトル列を計算します。
# クエリ Q、キー K、バリュー V です。それぞれは線形変換（全結合層）で求めます。
# - Q = X W_Q
# - K = X W_K
# - V = X W_V
#
...
# クンが、位置 j のトークンに対して持っている相関スコア」が入っています。
...
# 
# マスク付きスコア行列 Scores_masked を、行ごとに softmax にかけます。こうして得
# られる行列を注意重みと呼びます。
#
...
# 例えば位置5 のベクトル o_5 は、abcde 全体の情報を、モデルが重要だと判断した
# トークンほど強く反映したベクトルになります。c が文脈的に重要であれば
# Attention[5,3] が大きくなり、その分 v_3 が o_5 に強く効いてくる、というイメー
# ジです。
...
#
# Transformer ブロック内の FFN（Feed-Forward Network、位置ごとの全結合ネットワー
# ク）は、この文脈を埋め込んだベクトル o_i を各位置ごとに非線形に変換します。具
# 体的には、自己注意ブロックを通過した o_i が、1つ目のLayerNormと残差接続を通過
# したテンソルX_attn_norm が、形 (B, S, d_model) で FFN に入力されます。FFN の内
# 部は通常、1段目の線形変換として、d_model → dim_feedforward活性化関
...
# す。# ここでは、batch_first=True の入力テンソル (B, S, E) を前提とします。する
# と、同じトークンでも位置が違えば異なるベクトル表現になり、モデルは「内容」と
# 「位置」の両方を手がかりに文脈を処理し、次トークンの予測精度を高めることができ
# ます。ベクトルへの加算のイメージとしては、「犬」というトークンが高次元空間に位
# 置しているとき、そのベクトル（潜在空間上の点）の周辺も「犬」に対応する領域にな
# りますが、位置エンコーディングでは、その犬の意味を変えない程度にほんの少しだけ
# 位置を動かすることで、「トークン先頭の方の"犬"」という情報を与える、という感じ
# です。
#
...
# 出し、バッチ方向にブロードキャストして X (B, S, E) に加算します。 今回の
# batch_first = True の場合では、この加算は「テンソルの形を合わせて足すだけ」な
# ので実装は単純です。
###############################################################################
...
# く使う「softmax + クロスエントロピー」は、モデルが出したロジットから softmax
# でクラスごとの確率分布を作り、その中で正解クラスの確率だけを取り出して -log を
# 取る、という処理をまとめて一つの損失として実装したものです。PyTorch の
