...
「3の倍数 または 3 を含む数」を “アホ” と判定するだけのミニタスクを、Transformer で解かせるサンプル集です。  
...
- 学習（1〜4万を訓練、4万1〜5万を検証。100 epoch ごとに 10 epoch ごと保存）:
  ```bash
  python encoderonly.py
  ```
- 既存チェックポイントで推論だけ実行（1,000 個をランダム判定）:
  ```bash
  python encoderonly.py -c models/encoder_epoch_30.pth
  ```
- 対話モード（数字を入力すると即判定）:
  ```bash
  python encoderonly.py -c models/encoder_epoch_30.pth -i
  ```

### デコーダ版（`decoderonly.py`）
- 学習（1〜4万を訓練、4万1〜5万を検証。毎 epoch 保存。既定で 1000 epoch と長いので適宜短縮してください）:
  ```bash
  python decoderonly.py
  ```
- 推論だけ実行（1,000 個をランダム判定）:
  ```bash
  python decoderonly.py -c models/decoder_epoch_40.pth
  ```
- 対話モード:
  ```bash
  python decoderonly.py -c models/decoder_epoch_40.pth -i
  ```

## 実装のポイント
- ルールは `is_aho_number` に統一: 「3の倍数」または「桁に3を含む」なら True。
- トークナイザ:
