...
# 関係も扱えるようになります。
#
# 実装上は、あらかじめ最大系列長 max_len に対して (max_len, E) の位置エンコー
# ディング行列を作成しておき、順伝播時に入力の系列長 S に合わせて先頭 S 行を取り
...
# 層、4層とそうを積み上げてください。なお、最近のDeepSeekは64層のデコーダブロッ
# クを持っています。
# 
...
# 増えます。系列が 4倍になればだいたい16倍重くなるので、長い文書・コード・長尺の
# 音声や動画などを扱うとすぐ限界になります。これを回避するために、長さ n の系列
# の全数組み合わせ（n×n）を計算せずに、関連の強いトークンのみとのAttentionを計算
# する Sparse Attention などの改良方法が数多く提案されており、DeepSeekなども独自
# のSparse Attentionを採用しています。
...
class DecoderBlock(nn.Module):
    ...
    # dropout率について説明しておきます。ざっくり言うと、「dropout を下げる＝ラ
    # ンダムにニューロンを消す量を減らす」なので、正則化が弱くなってモデルが“素
    # の能力”を出しやすくなります。その結果として、まず学習時の挙動として
    # は、dropout を下げるとネットワークが受けるノイズが減るので、勾配が安定して
    # 訓練損失が下がりやすくなります。これまでdropout が強すぎて「そもそも十分に
    # フィットできていなかった」場合は、trainだけでなく val/test の精度も一緒に
    # 上がることがあります。いわゆるアンダーフィット気味だったのを解消する方向で
    # す。一方で、dropout は本来「ランダムにユニットを殺して、過学習を抑える」た
    # めの正則化なので、それを弱めるとモデルが訓練データにより強くフィットできる
    # ようになります。これは良い意味では表現力アップですが、データ量が少ない、ノ
    # イズが多い、パラメータ数がやたら多い、という状況だと「訓練精度だけ上がるけ
    # ど、検証・テスト精度はむしろ悪化する」という典型的なオーバーフィット方向の
    # 変化になりがちです。理屈としては、dropout が高いと「多数決を取るアンサンブ
    # ル」を擬似的にやっているような状態になり、個々のユニットに依存しすぎない頑
    # 丈な表現になります。これを下げると、そのアンサンブル効果が弱くなり、代わり
    # に特定の特徴や経路に依存した鋭い決定境界を作りやすくなります。そのぶん訓練
    # データには強いが、分布のズレやノイズには弱くなりがちです。実務的には、もと
    # もとの dropout が高すぎて lossが全体に高止まりしているなら下げるメリットが
    # 大きく、すでによくフィットしているモデルでさらに dropout を下げると、短期
    # 的には精度アップしても、数エポック後や別データでは悪化する、ということがよ
    # くあります。他の正則化（weight decay やデータ拡張など）とのバランスも変わ
    # るので、dropout を下げたら「train/val ロスとメトリクスの差がどう変わるか」
    # を一緒に見るのがおすすめです。
    ...
    def __init__(
        ...
        # クンを判断できます。 Feed-Forward Network（linear1 → activation →
        # dropout → linear2）は、アテンションの出力に対して位置ごとの非線形変換
        # を適用します。一度 dim_feedforward 次元まで拡張し、そこから d_model に
        # 再投影することで、非線形性を加えつつ、アテンションでコンテキストが混
        # ざった後の各トークンの特徴量を細かく整形します。より端的には、FFNは
        # 「Self-Attentionで収集した情報を、その位置ごとに“意味のある特徴”に変換
        # するための非線形な変換装置」です。まず Self-Attention で「どのトークン
        # がどのトークンからどれくらい情報をもらうか」が決まり（つまり、「Aho」
        # 判定という観点からの数字と数字の関係が決まり）、各位置に「コンテキスト
        # が混ざったベクトル」ができます。ただ、この段階では「足し合わせ+線形変
        # 換+softmax」程度の構造なので、表現力としては限定的です。そこで FFN
        # が、各位置のベクトルに対して独立に、しかし非線形な多層パーセプトロンと
        # して働き、 「このパターンが来たらこういう特徴を強調する／抑える」と
        # いった形で特徴を再構成します。学習という観点では、たとえば「この桁パ
        # ターンのときは [AHO] にしたい」 「この文脈のときは動詞の活用をこうす
        # る」といった、入力と出力の間の複雑な対応関係を、 FFN が各位置ごとの非
        # 線形写像としてモデル化していると言えます。 Self-Attention が「どの情報
        # を集めるか」を決める“ルーティング”の役割だとすると、 FFN は「集めてき
        # た情報からどういう決定境界・特徴抽出を学習するか」を担っている、と考え
        # ると分かりやすいと思います。また、実際の Transformer ではパラメータの
        # 大半が FFN に集中しており、ここがモデルの表現力を大きく左右していま
        # す。これまでの実験でも、Self-Attentionを残してFFNを削減すると性能が大
        # きく低下することが知られています。つまり、 「コンテキストを識別・合成
        # するのがSelf-Attention」で、 「コンテキスト依存のベクトルをタスクに役
        # 立つ形に整形・調整するのがFFN」 です。
    ...
    def forward(
        ...
        # 自己注意の計算です。 self.self_attn(x, x, x, ...) はクエリ・キー・バ
        # リューすべてに同じテンソル x（形: B, S, E）を使う「自己注意」。
        # attn_mask は (S, S) の因果マスクで、未来トークンを見ないよう上三角をマ
        # スクします。 key_padding_mask は (B, S) で PAD 位置を True にし、その
        # 位置を無視させます。 need_weights=False で注意重み行列を返さず、出力だ
        # け受け取ります。結果の attn_output（文脈を混ぜた特徴）を元の x に残差
        # 接続して、後段の LayerNorm へ渡しています。
        
        # 少し長くなりますが、自己注意機構を詳しく説明します。まず「クエリ・
        # キー・バリューすべてに同じテンソル x を使う」と書きましたが、自己注意
        # では、各トークンに対して「何を探すか」を表すベクトル（クエリ）、 「自
        # 分はどういう情報を提供できるか」を表すベクトル（キー）、 「実際に相手
        # に渡す中身」を表すベクトル（バリュー）、この三つを、それぞれ別々の「学
        # 習した変換ルール（バイアス）」を経由して計算します。つまり、変換ルール
        # が学習過程で動的に変化するので、たとえ元の入力ベクトル x が同じデータ
        # から来ていても、 QKVの全てが違う、という状態になります。また、「位置埋
        # め込み」の影響も重要です。文章で言うと「同じ単語でも、文頭にあるか文末
        # にあるかで意味合いが変わる」ので、モデルは各位置に違う“位置のラベル”を
        # 足してから自己注意に渡します。したがって、そもそも列の中の各トークン
        # は、内容も位置も含めた「その場所固有のベクトル」になっています。同じ x
        # という言い方をしていても、実際には“位置 i の x”と“位置 j の x”は別物で
        # す。これにより、数字の桁数をデコーダーが学習できるようになります。同じ
        # x を起源としても、最初にランダムに与えられた「クエリ用の変換バイア
        # ス」「キー用の変換バイアス」「バリュー用の変換バイアス」が、損失を小さ
        # くする方向にそれぞれ別々に調整され続けるため、「クエリ側は“何を探して
        # いるか”を強く表現するように」 「キー側は“どういうときに参照されるべき
        # か”が分かるように」 「バリュー側は“参照されたときに渡すべき情報”をうま
        # く持てるように」というように、自然と非対称な役割が形成されます。最初は
        # 何もできないけれど、誤差逆伝播を繰り返すうちに、「このタスクでは、こう
        # いうときにここのトークンを見に行くと正解になりやすい」というパターンが
        # 埋め込まれていきます。こうして、各位置のトークンが、自分のクエリを持っ
        # て、列の中のキーたちを見比べて、このトークンから情報を受け取る、この
        # トークンは無視する、と重みづけすることで、 “どこからコンテキストを集め
        # るか”が決まります。そのうえで、選ばれたトークンたちから「どんな特徴を
        # 受け取るか」はバリューが決め、その結果をあと段の Feed-Forward Network
        # がさらに非線形に加工します。
