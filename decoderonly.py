...
# 
# なお、本実装例は教育目的であり、実務での使用や大規模データセットでの学習にはそ
# のまま適用できません。大規模なトレーニングの場合は、効率化や安定化のために様々
# な工夫が必要です。
#
# このモデルのパラメタ数を書いておくと、合計で、101,903 パラメタ（0.1M）になりま
# す。Pythonなら1行で書けるAho数判定 `(n % 3 == 0) or ("3" in str(abs(int(n))))`
# を、10万パラメタのTransformerデコーダで学習するという非効率な例ですが、データ
# のみで学習するTransformerの基本を押さえるには良い例だと考えています。
#
# --------------------------------
# Embedding          =      960
# DecoderBlock 2層   =   99,968
# LM Head            =      975
# --------------------------------
# 合計               =  101,903 パラメタ
#
